...
3. 「負のホスピタリティ」を意識し、読者が速やかに嫌な気持ちになれるように
4. 五感に訴える不快な描写（特に聴覚、触覚、嗅覚）を含めてください
5. 読者が次にどう行動するか選択できる緊迫した状況で終わってください
    ...
    async def generate_response(self, quiz_answers: Dict[str, str], chat_history: List[Dict[str, str]]) -> str:
        ...
        try:
            ...
            if user_turns == 1:
                ...
                story_phase = "転"
                phase_instruction = "【転】恐怖のクライマックスへの転換点。予想外の展開や真実の一部を明かし、恐怖を最高潮に高めてください。"
            elif user_turns == 3:
                story_phase = "転"
            ...
            prompt = f"""
あなたは恐怖小説の専門的な語り部です。以下のユーザーの好みに基づいて、4ターン完結の短編ホラー小説を続けてください。
...
4. {'ユーザーの好みに応じた結末で物語を完結させてください。バッドエンド、ビターエンド、ハッピーエンドのいずれかで。' if is_final_turn else '読者が次にどう行動するか選択できる緊迫した状況で終わってください。'}
5. 五感に訴える不快な描写（聴覚、触覚、嗅覚）を積極的に使ってください
6. 「負のホスピタリティ」を意識し、読者が速やかに嫌な気持ちになれるように
